...
- **Generated icon set (Lagoon):** 16 soft-dimensional icons rendered with Qwen Image 2.1 on local ComfyUI (`icons/lagoon/`, transparent PNGs + raw sources in `icons/lagoon/raw/`). Replaces emoji for: header mascot, tab bar (cook/water/favorites/settings), goal rows (calories/protein/water/weight), pantry toggle, “Cook up ideas” button, 7 cuisine chips, and the meal-card favorite heart (greyed via CSS filter when off). PWA icons (192/512/apple-touch) re-rendered from the generated mascot. Caribbean/American/Surprise cuisines + diet chips stay emoji until a follow-up batch.
