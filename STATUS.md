...
- **Generated icon set (Lagoon theme only):** 16 soft-dimensional icons rendered with Qwen Image 2.1 on local ComfyUI (`icons/lagoon/`, transparent PNGs + raw sources in `icons/lagoon/raw/`). In the Lagoon theme they replace emoji for: header mascot, tab bar (cook/water/favorites/settings), goal rows (calories/protein/water/weight), pantry toggle, “Cook up ideas” button, 7 cuisine chips, and the meal-card favorite heart (greyed via CSS filter when off). Classic/Night keep the original emoji + SVG mascot — both variants live in the DOM and swap via `[data-theme]` CSS (instant switch, no JS, no flash); the original frog vector is kept as a hidden `<symbol id="froggy">` in `index.html`. PWA icons (192/512/apple-touch) re-rendered from the generated mascot. Caribbean/American/Surprise cuisines + diet chips stay emoji until a follow-up batch.
